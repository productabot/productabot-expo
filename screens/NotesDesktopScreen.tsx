...
export default function NotesScreen({ route, navigation, refresh }: any) {
    ...
    let onRefresh = async (showLoader = true, loadNewNote = false) => {
        ...
        let notesData = await API.graphql(graphqlOperation(`{
            notes(order_by: {order: desc}, where: {tag_id: {_eq: "${tag.id ? tag.id : tagsData.data.tags.length > 0 ? tagsData.data.tags[0].id : 'bb1871eb-929a-4a96-90e1-1ee7789e8872'}"}${search.length > 0 ? `, title: {_ilike: "%${search}%"}` : ``}}) {
              id
              title
            }
          }`));
                ...
                <View style={[{ width: '75%', height: '100%', borderWidth: 1, borderColor: '#444444', borderLeftWidth: 0 }]}>
                    ...
                    <ContentEditable
                        ...
                        onChange={(e) => {
                            ...
                            // you need to fix that bug where the caret gets placed at the end of the contenteditable everytime you save
                            // clearTimeout(timeout);
                            // timeout = setTimeout(() => { saveNote(); }, 5000);
                        }}
                        onBlur={async () => {
